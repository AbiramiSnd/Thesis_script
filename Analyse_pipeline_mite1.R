...
dim(N2)
# Compare 2 matrix
#N2 <- data.frame(N2, row.names = 1)
#M2 <- data.frame(M2, row.names = 1)
...
#N3=N2[N2$insertion %in% M2$insertion, ]
#rownames(M2)=M2$insertion
...
##
#M2[setdiff(names(N3), names(M2))] <- 0
#colnames(M2)
##
#M2=M2[ order(rownames(N3)), ]

#M2=M2[rownames(N3), colnames(N3)]
#M2[M2==0 & N3<2,]<-NA
##
